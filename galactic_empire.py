...
# travel speed to expand galactic empire
SPEED = 0.8 # fraction of speed of light

# time increment (in years) for expansion of galactic empire
INCREMENT = 2000
...
root = tk.Tk()
root.title("Building a Galactic Empire")
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    rot_fact = rotation factor
    fuz_fact = random shift in star position in arm, applied to 'fuzz' variable
    ...
    for x, y in spiral_stars:
        if arm == 0 and int(x % 2) == 0:
            ...
            c.create_oval(x-0.5, y-0.5, x+0.5, y+0.5, fill='white', outline='')
        elif arm == 1:
...
def expand_empire(x, y, speed, increment):
    years = []
    cum = 0
    y_text = -270
    for i in range(10):
        cum += increment
        years.append(cum)
    c.create_oval(x-1, y-1, x+1, y+1, fill='', outline='white')
    for year in years:
        c.create_oval(x-speed*year/SCALE, y-speed*year/SCALE, x+speed*year/SCALE, y+speed*year/SCALE, fill='', outline='white')
        c.create_text(-455, y_text, fill='white', anchor='w',
                      text=f'Years= {year:,}')
        y_text+=20

def main():
    """Calculate detection probability & post galaxy display and statistics."""
    ...
    c.create_text(-455, -330, fill='white', anchor='w',
                  text='Increment = {:,} years'.format(INCREMENT))
    ...
    if SCALE == 225:
        ...
        # Create start of expanding radius around earth
        # c.create_oval(100, 60, 130, 90, fill='', outline='white')
    expand_empire(115, 75, SPEED, INCREMENT)
    # run tkinter loop
    root.mainloop()
