...
class Satellite(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.dx == 0 and self.dy == 0:
            ...
            self.image_sat.set_colorkey(BLACK)
...
def instruct_label(screen, text, color, x, y):
    """Take screen, list of strings, color, and origin & render text to screen."""
...
def intro_screen(screen):
    intro_image = pg.image.load("mars_mission_logo.png").convert()
    intro_image_small = pg.transform.scale(intro_image, (200, 150))
    screen.blit(intro_image_small, (300, 100))
...
def main():
    ...
    while running:
        ...
        # display intro image
        if pg.time.get_ticks() <= 10000:  # time in milliseconds
            intro_screen(screen)

        if pg.time.get_ticks() > 10000 and pg.time.get_ticks() <= 20000:  # time in milliseconds
        ...
        box_label(screen, '{:.1f}'.format(eccentricity), (580, 50, 150, 20))
